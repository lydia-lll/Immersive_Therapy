...
public class MyBrush : MonoBehaviour
{
    ...
    // public GameObject handR;
    // public GameObject handL;
    // private GameObject handR = transform.Find("XR Origin/Camera Offset/RightHand Controller");
    ...
    void Update() {
        ...
        if (triggerPressed && _activeBrushStroke == null) {
            ...
            _activeBrushStroke.BeginBrushStrokeWithBrushTipPoint(handR.transform.position, handR.transform.rotation);
        }

        // If the trigger is pressed, and we have a brush stroke, move the brush stroke to the new brush tip position
        if (triggerPressed)
            _activeBrushStroke.MoveBrushTipToPoint(handR.transform.position, handR.transform.rotation);

        // If the trigger is no longer pressed, and we still have an active brush stroke, mark it as finished and clear it.
        if (!triggerPressed && _activeBrushStroke != null) {
            _activeBrushStroke.EndBrushStrokeWithBrushTipPoint(handR.transform.position, handR.transform.rotation);
    ...
    static bool UpdatePose(XRNode node, ref Vector3 position, ref Quaternion rotation) {
        ...
        foreach (XRNodeState nodeState in nodeStates) {
            if (nodeState.nodeType == node) {
                ...
                // if (gotPosition)
                //     position = handR.transform.position;
                // if (gotRotation)
                //     rotation = handR.transform.rotation;
